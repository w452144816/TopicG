...
- 话题 / 回复 / 画像的展示区右上角有复制图标，一键复制全文。
- 话题页「再来一批」会避开前几批已生成的话题；话题 / 回复都可选中某一条按要求改写（更简短、更正式、自定义要求等），改写仍是你本人的口吻。
- 多行输入框（客户消息、文字记录、我的自述）按 **Shift+Enter** 直接提交；对话模拟单行框回车发送。
- 顶部引擎状态折叠成一行，点开可看详情并临时覆盖 Key / 模型 / 代理。
- 手机浏览器打开时并排区块自动上下堆叠，可凑合使用。
- 分析页与我的建模页画像下方有「手工修正画像」：直接改 JSON 字段后保存，之后的重新分析会在你改过的版本上增量更新。
- AI 请求进行中相关按钮会置灰，避免重复点击；删除材料改为下拉选择，不用数序号。
- 新增客户后自动切换到「资料录入」页。
...
| Tab | 作用 |
|---|---|
| 使用说明 | 页面内说明书：工具流程、五步上手、我的建模与客户建模各需要哪些数据（优先级表 + 自述模板）、各页面操作说明（带截图，截图在 `docs/img/`）、常见问题与数据存放说明 |
